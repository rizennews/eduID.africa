...
interface InstitutionsCtaSectionProps {
  locale: Locale;
  dict: {
    institutionsPage: {
      ctaSection: {
        ...
        description: string;
        btnRoadshows: string;
        btnAssessment: string;
      };
    };
  };
}

export function InstitutionsCtaSection({ locale, dict }: InstitutionsCtaSectionProps) {
  const { ctaSection } = dict.institutionsPage;
    ...
    <section className="py-16 sm:py-20 bg-[#F8FAFC] border-b border-dashed border-slate-300">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          ...
          >
            <span>{ctaSection.btnRoadshows}</span>
          ...
          >
            <span>{ctaSection.btnAssessment}</span>
          </Link>
        </div>
      </div>
    </section>
